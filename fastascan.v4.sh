...
length=$(sed -e '/^>.*/ d' $i | awk -b '{sum += length($0)} END {print sum}')
...
echo "Total number of sequences: " $(awk '(NR>1){sum+=$2}END{print sum}' table.FastaScan.tbl)
echo "Total length of the sequences: " $(awk '(NR>1){sum+=$3}END{print sum}' table.FastaScan.tbl)
echo 
echo "A random title: " 
